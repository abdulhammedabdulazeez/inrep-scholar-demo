...
export default function ReviewTasksPage() {
  ...
  const handleSubmitReview = () => {
    ...
    setReviewData({ recommendation: '', comments: '', confidentialComments: '' });
  };
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-gray-600">Manage your assigned document reviews and provide feedback to authors</p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
              ...
              <div>
                ...
                <ul className="space-y-1">
                  ...
                  <li>• Recommend: Approve, Minor Revision, Major Revision, or Reject</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {reviewTasks.map((task) => {
              ...
              return (
                <div key={task.id} className="bg-white rounded-lg shadow border p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      ...
                      <h3 className="text-xl font-semibold text-gray-900 mb-2">
                        <Link
                          href={`/demo/ur/documents/${task.documentId}`}
                      ...
                      <p className="text-gray-600 mb-3">
                        by <strong>{task.author}</strong> • {task.faculty} • {task.department}
                      </p>

                      <p className="text-gray-700 line-clamp-2 mb-4">{task.abstract}</p>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-500">
                        ...
                        <div>
                          ...
                          <span className={`ml-1 ${daysInfo.status === 'overdue' ? 'text-red-600' : daysInfo.status === 'urgent' ? 'text-orange-600' : ''}`}>
                            {task.deadline} ({daysInfo.days} days {daysInfo.status === 'overdue' ? 'overdue' : 'remaining'})
                          </span>
                        </div>
                        <div>
                          <span className="font-medium">Pages:</span> {task.pages}
                        </div>
                        <div>
                          <span className="font-medium">File Size:</span> {task.fileSize}
                        </div>
                      </div>
                      ...
                      {task.status === 'completed' && (
                        <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-md">
                          <div className="flex items-center space-x-2">
                            <span className="text-green-600 font-medium">✓ Review Completed</span>
                            <span className="text-sm text-gray-600">on {task.completedDate}</span>
                            <span className="text-sm text-gray-600">• Recommendation: <strong>{task.recommendation}</strong></span>
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <div className="bg-white rounded-lg max-w-4xl w-full mx-4 max-h-screen overflow-y-auto">
                <div className="p-6 border-b">
                  ...
                  <p className="text-sm text-gray-600">
                    Document: {reviewTasks.find(t => t.id === selectedTask)?.title}
                  </p>
                </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  {/* Comments for Author */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Comments for Author *</label>
                    <p className="text-xs text-gray-500 mb-2">These comments will be shared with the author</p>
                    <textarea
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Confidential Comments for Admin</label>
                    <p className="text-xs text-gray-500 mb-2">These comments are only visible to administrators</p>
                    <textarea
                ...
                <div className="p-6 border-t flex justify-end space-x-3">
                  ...
                  <button
                    ...
                    disabled={!reviewData.recommendation || !reviewData.comments}
