...
export default function DocumentUploadPage() {
  ...
  const [abstractExtractionStatus, setAbstractExtractionStatus] = useState<'pending' | 'extracting' | 'success' | 'failed' | null>(null);
  const [plagiarismStatus, setPlagiarismStatus] = useState<'pending' | 'checking' | 'passed' | 'failed' | null>(null);
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <nav className="flex items-center space-x-4">
              <Link href="/demo/ur/user" className="text-gray-600 hover:text-blue-600">My Dashboard</Link>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-center space-x-8">
            ...
            <div className={`w-16 h-1 ${currentStep >= 2 ? 'bg-blue-600' : 'bg-gray-300'} rounded`} />
            ...
            <div className={`w-16 h-1 ${currentStep >= 3 ? 'bg-blue-600' : 'bg-gray-300'} rounded`} />
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition">
                    {!file ? (
                      <>
                        ...
                        <p className="text-sm text-gray-500">Supports PDF, DOCX (max 50MB)</p>
                        <input
                          type="file"
                          accept=".pdf,.docx,.doc"
                ...
                {file && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div className="flex items-center space-x-3">
                      <span className="text-2xl">{getStatusIcon(abstractExtractionStatus)}</span>
                      <div>
                        <h4 className="font-medium text-gray-900">Abstract Extraction</h4>
                        <p className={`text-sm ${getStatusColor(abstractExtractionStatus)}`}>
                          {abstractExtractionStatus === 'extracting' && 'Analyzing document and extracting abstract...'}
                          {abstractExtractionStatus === 'success' && 'Abstract successfully extracted! Please review and edit if needed.'}
                          {abstractExtractionStatus === 'failed' && 'Failed to extract abstract. Please enter manually.'}
                          {!abstractExtractionStatus && 'Waiting for file upload...'}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  {abstractExtractionStatus === 'success' && (
                    <p className="text-sm text-green-600 mb-2">✅ Auto-extracted from your document. Please review and edit if needed.</p>
                  )}
                  <textarea
                    ...
                    rows={6}
                    className="w-full border border-gray-300 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder={abstractExtractionStatus === 'extracting' ? 'Extracting abstract from document...' : 'Enter the document abstract'}
                    required
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, accessRights: e.target.value }))}
                      className="w-full border border-gray-300 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                      required
                    >
                      <option value="open_access">Open Access (Public)</option>
                      <option value="restricted">Restricted (University Only)</option>
                  ...
                  {formData.accessRights === 'open_access' && (
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, ccLicense: e.target.value }))}
                        className="w-full border border-gray-300 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-3">
                    <input
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, hasExistingDOI: e.target.checked }))}
                      className="w-4 h-4 text-blue-600"
                    />
                    <label htmlFor="hasExistingDOI" className="text-sm font-medium text-gray-700">
                  ...
                  {formData.hasExistingDOI && (
                    <input
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, existingDOI: e.target.value }))}
                ...
                <div className="flex justify-end space-x-4">
                  ...
                  <button
                    type="submit"
                    disabled={!file || isSubmitting || abstractExtractionStatus === 'extracting'}
              ...
              <div className="space-y-6">
                ...
                <div className="border rounded-lg p-6">
                  <div className="flex items-center space-x-4 mb-4">
                    <span className="text-3xl">{getStatusIcon(plagiarismStatus)}</span>
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">Plagiarism Check</h3>
                      <p className={`${getStatusColor(plagiarismStatus)}`}>
                        {plagiarismStatus === 'checking' && 'Analyzing document for similarity... This may take a few minutes.'}
                        {plagiarismStatus === 'passed' && 'Plagiarism check passed! Similarity score: 8.2% (Below 20% threshold)'}
                        {plagiarismStatus === 'failed' && 'Plagiarism check failed. High similarity detected. Please review and revise.'}
                      </p>
                    </div>
                  </div>
                ...
                {plagiarismStatus === 'passed' && (
                  <div className="border rounded-lg p-6">
                    <div className="flex items-center space-x-4 mb-4">
                      ...
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900">Peer Review Assignment</h3>
                        <p className="text-gray-600">Your document will be assigned to reviewers for academic evaluation.</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {plagiarismStatus === 'passed' && (
                  <div className="border rounded-lg p-6">
                    <div className="flex items-center space-x-4 mb-4">
                      <span className="text-3xl">⏳</span>
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900">DOI Assignment</h3>
                        <p className="text-gray-600">A Digital Object Identifier will be assigned after peer review approval.</p>
                      </div>
                    </div>
                  </div>
                )}
        ...
        {currentStep === 3 && (
          <div className="text-center">
            <div className="bg-white rounded-lg shadow border p-8">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Document Successfully Submitted!</h2>
              <p className="text-gray-600 mb-6">
                Your document has been uploaded and passed the initial quality checks.
                It will now go through the review process.
              </p>

              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <h3 className="font-semibold text-gray-900 mb-2">Next Steps:</h3>
                <ul className="text-sm text-gray-700 space-y-1">
                  <li>• Reviewers will be assigned to evaluate your document</li>
                  <li>• You'll receive email notifications about review progress</li>
